...
namespace Munientry.Poc.Api.Tests
{
    public class SentencingOnlyAlreadyPleadApiTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        [Fact]
        public async Task PostSentencingOnlyAlreadyPlead_ReturnsDocx()
        {
            var client = _factory.CreateClient();
            var dto = new SentencingOnlyAlreadyPleadDto
            {
                DefendantFirstName = "Test",
                DefendantLastName = "User",
                CaseNumber = "2026-CR-013",
            ...
            var response = await client.PostAsJsonAsync("/api/sentencingonlyalreadypead", dto);
